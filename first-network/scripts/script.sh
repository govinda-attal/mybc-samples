...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		...
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n marbles -v 1.0 -c '{"Args":["init","a","100","b","200"]}' -P "OR	('Org1MSP.member')" >&log.txt
	else
		...
		peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n marbles -v 1.0 -c '{"Args":["init","a","100","b","200"]}' -P "OR	('Org1MSP.member')" >&log.txt
	fi
	res=$?
	cat log.txt
